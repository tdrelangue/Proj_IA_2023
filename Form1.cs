...
namespace ProjetIA2022
{
    public partial class Form1 : Form
    {
        ...
        static public int xbouee1;  // ces 4 int ne sont utilisés que pour le pb de passage par les 2 bouées
        static public int ybouee1;
        static public int xbouee2;
        static public int ybouee2;
        ...
        public void load_environment(string filename)
        {
            ...
            while (ligne != null)
            {
                ...
                {
                    obstacle = false;           // sinon, il est écrit "bouee" dans le fichier,
                                                //  c'est donc un b
                    numerobouee++;
                }
                ...
                {
                    matrice[xobstacle, yobstacle] = -1; // bouée
                    if (numerobouee == 1)
                    { xbouee1 = xobstacle; ybouee1 = yobstacle; }
                    else
                    { // 2 bouées prises en compte uniquement
                        xbouee2 = xobstacle; ybouee2 = yobstacle;
                    }
                }
        ...
        {
            StartAEtoilePbBouees("environnement2.txt");
        }
        private void button3_Click(object sender, EventArgs e)
        {
            StartAEtoilePbBouees("environnement3.txt");
        }
        ...
        private void buttonAstar_Click(object sender, EventArgs e)
        {
            ...
            labeltpstotal.Text = Convert.ToString(tpstotal);

        }

        private void StartAEtoilePbBouees ( string env )
         {
            ...
            for (int i = 1; i < solution.Count; i++)
            {
                Node3 N2 = (Node3)solution[i];
                //            listBox1.Items.Add(Convert.ToString(N1.x) + "," + Convert.ToString(N1.y)
                //                 + " ---> " + Convert.ToString(N2.x) + "," + Convert.ToString(N2.y));
                listBox1.Items.Add(Convert.ToString(N1)+ " ---> " + Convert.ToString(N2) );
                            rect = new Rectangle(N2.x * largeur, N2.y * hauteur, largeur - 1, hauteur - 1);
                if (matrice[N2.x, N2.y] == 0)
                    g.FillRectangle(brush3, rect);
                else g.FillRectangle(brush4, rect);
            ...
            labelOuverts.Text = Convert.ToString(search.L_Ouverts.Count);
            labelFermes.Text = Convert.ToString(search.L_Fermes.Count);
            int tpstotal = (int)(((Node3)solution[solution.Count - 1]).GetGCost());
            labeltpstotal.Text = Convert.ToString(tpstotal);
